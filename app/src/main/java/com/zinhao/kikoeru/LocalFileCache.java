...
//        }
    ...
    public boolean getLrc(Context context,String hash,AsyncHttpClient.JSONObjectCallback callback){
        String[] strings = hash.split("/");
        if(strings.length == 2){
            String idStr = strings[0];
            String positionStr = strings[1];
            File workDir = instance.getExternalCacheDir(context,Integer.parseInt(idStr));
        }else {
            return false;
        }
        return false;
            ...
            public void run() {
                ...
                if(workJsonDir.exists()){
                    ...
                    try {
                        ...
                        callback.onCompleted(null,null,rootJson);
                ...
                if(workTreeFile.exists()){
                    ...
                    if(workStr != null && !workStr.isEmpty() && workStr.startsWith("[")){
                        try {
                            ...
                            callback.onCompleted(null,null,workTree);
                        } catch (JSONException e) {
                            e.printStackTrace();
